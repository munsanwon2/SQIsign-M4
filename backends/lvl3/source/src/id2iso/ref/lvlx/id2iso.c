                        ...
                        const ec_curve_t *curve)
{
    ...
    if (res == NULL || scalar_vec == NULL || PQ == NULL || curve == NULL ||
        ...
        !ibz_to_mp_digits_checked(scalars[0], NWORDS_ORDER, &(*scalar_vec)[0]) ||
        !ibz_to_mp_digits_checked(scalars[1], NWORDS_ORDER, &(*scalar_vec)[1]))
        return 0;
...
matrix_application_even_basis(ec_basis_t *bas, const ec_curve_t *E, ibz_mat_2x2_t *mat, int f)
{
    ...
    if (!ibz_to_mp_digits_checked(scalars[0], NWORDS_ORDER, &(*mat)[0][0]) ||
        !ibz_to_mp_digits_checked(scalars[1], NWORDS_ORDER, &(*mat)[1][0]) ||
        !ec_biscalar_mul(&candidate.P, scalars[0], scalars[1], f, &tmp_bas, E))
        goto cleanup;

    // second basis element S = [c]P + [d]Q
    if (!ibz_to_mp_digits_checked(scalars[0], NWORDS_ORDER, &(*mat)[0][1]) ||
        !ibz_to_mp_digits_checked(scalars[1], NWORDS_ORDER, &(*mat)[1][1]) ||
    ...
    if (!ibz_to_mp_digits_checked(scalars[0], NWORDS_ORDER, &tmp))
        goto cleanup;
    ibz_sub(&tmp, &(*mat)[1][0], &(*mat)[1][1]);
    ibz_mod(&tmp, &tmp, &pow_two);
    if (!ibz_to_mp_digits_checked(scalars[1], NWORDS_ORDER, &tmp) ||
