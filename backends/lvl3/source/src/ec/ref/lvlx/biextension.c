...
{
    enum { FP2_DLOG_MAX_E = 512, FP2_DLOG_MAX_DEPTH = 10 };
    if (scal == NULL || f == NULL || g_inverse == NULL ||
        e <= 0 || e > FP2_DLOG_MAX_E)
        return false;
    ...
    if (log > FP2_DLOG_MAX_DEPTH)
        return false;
    fp2_t pows_f[FP2_DLOG_MAX_DEPTH], pows_g[FP2_DLOG_MAX_DEPTH];
    pows_f[0] = *f;
    pows_g[0] = *g_inverse;
