...
/** @returns 1 on success, 0 if compact lattice addition failed. On failure,
 * res is unchanged. */
int quat_lattice_intersect(quat_lattice_t *res,
 ...
 * @returns 1 on success, 0 if compact lattice reduction failed. On failure,
 *          lideal is unchanged.
 */
int quat_lideal_create(quat_left_ideal_t *lideal,
                       const quat_alg_elem_t *x,
                       const ibz_t *N,
                       const quat_lattice_t *order,
                       const quat_alg_t *alg);
 ...
 * the 2*coord+p_bits transient blow-up in quat_alg_norm. Returns 1 on
 * success and 0, with lideal unchanged, on fixed-precision failure. */
int quat_lideal_create_with_norm(quat_left_ideal_t *lideal,
                                 const quat_alg_elem_t *x,
                                 const ibz_t *N,
                                 const quat_lattice_t *order,
                                 const quat_alg_t *alg);
 ...
 * @param alg the quaternion algebra
 */
int quat_lideal_conjugate_without_hnf(quat_left_ideal_t *conj,
 ...
 * @param lideal2 left ideal
 * @param alg the quaternion algebra
 * @returns 1 on success, 0 if the compact full-rank lattice reduction failed.
 *          On failure, intersection is left unchanged.
 */
int quat_lideal_inter(quat_left_ideal_t *intersection,
 ...
 *
 * @returns 1 on success, 0 with product unchanged on fixed-precision failure
 */
int quat_lideal_mul(quat_left_ideal_t *product,
                    const quat_left_ideal_t *lideal,
 ...
 * @returns QUAT_RANDOM_IDEAL_SUCCESS on success,
 *          QUAT_RANDOM_IDEAL_RETRY for an ordinary stochastic miss, or
 *          QUAT_RANDOM_IDEAL_FATAL if fixed-precision construction failed
 *          after all local ML2 attempts.  The fatal result must not be turned
 *          into a retry of the surrounding protocol.
 */
typedef enum {
    QUAT_RANDOM_IDEAL_FATAL = -1,
    QUAT_RANDOM_IDEAL_RETRY = 0,
    QUAT_RANDOM_IDEAL_SUCCESS = 1,
} quat_random_ideal_status_t;

quat_random_ideal_status_t quat_sampling_random_ideal_O0_given_norm(
...
/**
 * @brief Finds two Ii=small_equiv*conj(mui)/norm(small_equiv)~ lideal such that norm(I1)+norm(I2)= 2^two_power
 *
 * In this small_equiv denotes (lideal*conj(smallest/norm(lideal)).
 * Sometimes it outputs instead a solution for (two-power -1). This is indicated by the return value.
 *
 * @param mu1 Output: mu1 such that I1 = (lideal*conj(smallest/norm(lideal)) conj(mu1)/norm(small_equiv)
 * @param mu2 Output: mu2 such that I2 = (lideal*conj(smallest/norm(lideal))* conj(mu2)/norm(small_equiv)
 * @param theta Output: mu2*conj(mu1)/norm
 * @param smallest Output: Smallest element in lideal
 * @param lideal Input ideal. Must be left ideal of a special extremal maximal order
 * @param alg The quaternion algebra
 * @param cornacchia_params Parameters for extended cornacchia with q the one from the special extremal order of which
 * lideal is a left order
 * @param max_counter_alpha Number of generators of a small equivalent of lideal used
 * @param gen_sampling_bound_bits Size in bits of a side of the box in which generators are randomly sampled
 * @param two_power t such that 2^t=norm(I1)+norm(I2)
 * @returns QUAT_QLAPOTI_SUCCESS or QUAT_QLAPOTI_SUCCESS_TRANSFORMED on
 * success, QUAT_QLAPOTI_RETRY for an ordinary bounded search miss, and
 * QUAT_QLAPOTI_FATAL for invalid input or fixed-precision failure.
 */
typedef enum {
    QUAT_QLAPOTI_FATAL = -1,
    QUAT_QLAPOTI_RETRY = 0,
    QUAT_QLAPOTI_SUCCESS = 1,
    QUAT_QLAPOTI_SUCCESS_TRANSFORMED = 3,
} quat_qlapoti_status_t;

int quat_qlapoti(quat_alg_elem_t *mu1,
                 quat_alg_elem_t *mu2,
