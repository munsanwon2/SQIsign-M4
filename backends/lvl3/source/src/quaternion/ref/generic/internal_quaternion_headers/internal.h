...
void ibz_mat_2x2_add(ibz_mat_2x2_t *sum, const ibz_mat_2x2_t *a, const ibz_mat_2x2_t *b); // unused

void ibz_2x2_mul(ibz_mat_2x2_t *prod, const ibz_mat_2x2_t *mat_a, const ibz_mat_2x2_t *mat_b);
 ...
 *
 * Compact lattice operations may return any full-rank basis of the lattice.
 * This helper canonicalizes such a basis when HNF is specifically required.
 */
int quat_lattice_hnf(quat_lattice_t *lat); // lattice, test/lattice, test/algebra,

/**
 * @brief Lattice equality
 *
 * Inputs may use arbitrary full-rank bases; they are canonicalized internally.
 *
 ...
 *
 * Inputs may use arbitrary full-rank bases; denominators are unrestricted.
 *
 ...
 * @param lat Lattice
 */
int quat_lattice_reduce_denom(quat_lattice_t *reduced,
 ...
 *
 * @param dual Output: The dual lattice of lat. The basis is not necessarily
 * in HNF, which is a valid Compact lattice representation. Unchanged on
 * failure.
 * @param lat lattice, the dual of it will be computed
 * @returns 1 on success, 0 for an invalid/singular representation or
 * arithmetic failure
 */
int quat_lattice_dual_without_hnf(quat_lattice_t *dual,
                                   const quat_lattice_t *lat); // lattice, ideal
 ...
 *
 * @param index Output
 * @param sublat A full-rank sublattice of overlat
 * @param overlat A full-rank overlattice of sublat
 * @returns 1 on success, 0 with index unchanged for an invalid representation
 * or non-integral quotient
 */
int quat_lattice_index(ibz_t *index, const quat_lattice_t *sublat,
 ...
 * @returns 1 on success, 0 for an invalid ideal representation
 */
int quat_lideal_norm(quat_left_ideal_t *lideal); // ideal
 ...
 *
 * @returns 1 on success, 0 with lideal unchanged on failure
 */
int quat_lideal_create_principal(quat_left_ideal_t *lideal,
 ...
 * @param alg the quaternion algebra
 */
int quat_lideal_class_gram(ibz_mat_4x4_t *G, const quat_left_ideal_t *lideal, const quat_alg_t *alg);
 ...
 */
/**  @internal
 * @defgroup quat_qlapoti_helper Helper functions for qlapoti
 * @{
 */
/** @brief Type for parameters used in the cvp condition for qlapoti
 *
 * @typedef qlapoti_enumeration_parameters_t
 *
 * @struct qlapoti_enumeration_parameters
 *
 */
typedef struct qlapoti_enumeration_parameters
{
    const ibz_t *m;
    const ibz_t *n;
    const ibz_t *a_alpha; ///< double of a_alpha
    const ibz_t *b_alpha; ///< double of b_alpha
    const ibz_cornacchia_extended_params_t
        *cornacchia_params; ///< parameters for extended cornacchia. q must correspond to the order in which we are
                            ///< solving (1 for O0)
} qlapoti_enumeration_parameters_t;

/**
 * @brief  Shortest element and shortest equivalent ideal of lideal
 *
 * @param equiv Output: Shortest ideal equivalent to lideal
 * @param elem Output: A shrtest element of equiv used to generate elem
 * @param lideal left ideal
 * @param alg the quaternion algebra
 * @returns 1 on success, 0 with both outputs unchanged on failure
 */
int quat_lideal_shortest_equivalent(quat_left_ideal_t *equiv,
                                    quat_alg_elem_t *elem,
                                    const quat_left_ideal_t *lideal,
                                    const quat_alg_t *alg);

/** @brief  Finds a generator of 'lideal' with 2*first coordinate invertible mod lideal.norm
 *
 * @param gen Output: generator of lideal, coprime to n
 * @param lideal left ideal, assumedfor short outputs it must be given with a reduced basis
 * @param sampling_bound_bits generators are sampled in a box of 2^sampling_bound_bits (for a reduced bases of lideal)
 * @param max_attempts hard upper bound on random samples
 * @param alg the quaternion algebra
 * @returns 1 on success, 0 on invalid input, RNG failure, or exhaustion
 */
/** Returns QUAT_QLAPOTI_SUCCESS, QUAT_QLAPOTI_RETRY after exhausting the
 * bounded random search, or QUAT_QLAPOTI_FATAL for invalid input/RNG or a
 * fixed-precision invariant failure. */
int quat_lideal_generator_small_coprime(quat_alg_elem_t *gen,
                                        const quat_left_ideal_t *lideal,
                                        const quat_alg_t *alg,
 ...
 * @param norm_q Positive integer
 */
void quat_dim2_lattice_bilinear(ibz_t *res,
                                const ibz_t *v11,
                                const ibz_t *v12,
                                const ibz_t *v21,
                                const ibz_t *v22,
...
/** @brief Basis of the integral lattice represented by basis, which is small for the norm x^2 + qy^2 (in standard
 * basis)
 *
 ...
 *
 * @param reduced Output: Matrix of 2 column vectors of small norm x^2 + qy^2 which are a basis of the lattice the basis
 * argument represents
 * @param basis Basis of a rank 2 lattice in dimension 2
 * @param norm_q Positive integer defining a quadratic form x^2 + qy^2 (x,y coordinates of a vector in the standard
 * basis of R^2) for which the reduced output basis shound be small
 */
void quat_dim2_lattice_short_basis(ibz_mat_2x2_t *reduced, const ibz_mat_2x2_t *basis, const ibz_t *norm_q);

int quat_dim2_lattice_qlapoti_cvp_condition(quat_alg_elem_t *elem, const ibz_vec_2_t *vec, const void *params);

/**  @internal
 * @ingroup quat_qlapoti_helper
