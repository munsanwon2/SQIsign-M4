...
enum {
    THETA_CHAIN_MAX_LENGTH = 512,
    THETA_CHAIN_JAC_SLOTS = 8,
    THETA_CHAIN_THETA_SLOTS = 9,
    THETA_CHAIN_MAX_POINTS = 3
};

/* These four fixed arrays account for most of the theta-chain frame (25,920
 * bytes at level V).  The unchanged gluing strategy needs at most eight
 * Jacobian slots; the offline ordinary strategy needs at most nine theta
 * slots.  Cortex-M4 operations place them in the bounded CCM arena; host
 * builds deliberately retain automatic storage so independent signing
 * threads do not share mutable state. */
typedef struct theta_chain_workspace {
    theta_point_t pts[THETA_CHAIN_MAX_POINTS];
    theta_couple_jac_point_t jacQ1[THETA_CHAIN_JAC_SLOTS];
    theta_couple_jac_point_t jacQ2[THETA_CHAIN_JAC_SLOTS];
    theta_point_t thetaQ1[THETA_CHAIN_THETA_SLOTS];
    theta_point_t thetaQ2[THETA_CHAIN_THETA_SLOTS];
} theta_chain_workspace_t;

#ifdef TARGET_ARM
#if BITS == 256
_Static_assert(sizeof(theta_chain_workspace_t) == 12960,
               "level-I theta-chain CCM workspace size changed");
#elif BITS == 384
_Static_assert(sizeof(theta_chain_workspace_t) == 20160,
               "level-III theta-chain CCM workspace size changed");
#elif BITS == 512
_Static_assert(sizeof(theta_chain_workspace_t) == 25920,
               "level-V theta-chain CCM workspace size changed");
#else
#error "unsupported field size for the audited theta-chain workspace"
#endif
#if defined(__GNUC__) || defined(__clang__)
#define THETA_CHAIN_NOINLINE __attribute__((noinline))
#else
#define THETA_CHAIN_NOINLINE
#endif
#else
#define THETA_CHAIN_NOINLINE
#endif
...
// splitting
static bool
sample_random_index(unsigned char *secret_index_out)
{
    ...
    for (unsigned attempts = 0; attempts < 16; ++attempts) {
        if (sqisign_randombytes(seed_arr, sizeof(seed_arr)) != 0) {
            sqisign_randombytes_wipe(seed_arr, sizeof(seed_arr));
            sqisign_randombytes_wipe(&seed, sizeof(seed));
            return false;
        }
        seed = (uint32_t)seed_arr[0] |
               ((uint32_t)seed_arr[1] << 8) |
               ((uint32_t)seed_arr[2] << 16) |
               ((uint32_t)seed_arr[3] << 24);
        sqisign_randombytes_wipe(seed_arr, sizeof(seed_arr));
        if (seed < UINT32_C(4294967292)) {
            uint32_t secret_index = seed - (((uint64_t)seed * UINT32_C(2863311531)) >> 34) * 6;
            assert(secret_index == seed % 6); // ensure the constant time trick above works
            *secret_index_out = (unsigned char)secret_index;
            sqisign_randombytes_wipe(&seed, sizeof(seed));
            return true;
        }
        sqisign_randombytes_wipe(&seed, sizeof(seed));
...

{
    ...
    if (randomize) {
        unsigned char secret_index;
        if (!sample_random_index(&secret_index))
            return false;
...
static THETA_CHAIN_NOINLINE int
theta_chain_compute_with_workspace(unsigned n,
                                   ...
                                   bool verify,
                                   bool randomize,
                                   theta_chain_workspace_t *workspace)
{
    /* Without extra torsion, n=1/2 make todo[0] invalid and n=3 reaches
     * the fixed tail without having populated thetaQ1[0]/thetaQ2[0]. */
    if (n == 0 || n > THETA_CHAIN_MAX_LENGTH || E12 == NULL ||
        ker == NULL || E34 == NULL || numP > THETA_CHAIN_MAX_POINTS ||
        (numP != 0 && P12 == NULL) || workspace == NULL ||
        (!extra_torsion && n < 4))
        return 0;
    ...
    theta_point_t *const pts = workspace->pts;

    uint16_t todo[THETA_CHAIN_THETA_SLOTS];
    todo[0] = n - 2 + extra;

    int current = 0;

    // kernel points for the gluing isogeny
    theta_couple_jac_point_t *const jacQ1 = workspace->jacQ1;
    theta_couple_jac_point_t *const jacQ2 = workspace->jacQ2;
    jacQ1[0] = xyT1;
    jacQ2[0] = xyT2;
    while (todo[current] != 1) {
        ...
        const unsigned remaining = todo[current];
        const unsigned num_dbls = remaining >= 16 ? remaining / 2 : remaining - 1;
        if (num_dbls == 0 || num_dbls >= remaining ||
            current + 1 >= THETA_CHAIN_JAC_SLOTS)
            return 0;
        ++current;
        ...
        todo[current] = remaining - num_dbls;
    }

    // kernel points for the remaining isogeny steps
    theta_point_t *const thetaQ1 = workspace->thetaQ1;
    theta_point_t *const thetaQ2 = workspace->thetaQ2;

    /* The gluing, ordinary-isogeny and splitting descriptions are used in
     * three disjoint phases.  An explicit union prevents the compiler from
     * reserving all three address-taken objects for the whole function. */
    union {
        theta_gluing_t gluing;
        theta_isogeny_t isogeny;
        theta_splitting_t splitting;
    } chain_step;

    // the gluing step
    {
        if (current < 0 || current >= THETA_CHAIN_JAC_SLOTS || todo[current] != 1)
            return 0;

        // compute the gluing isogeny
        if (!gluing_compute(&chain_step.gluing, E12, &jacQ1[current], &jacQ2[current], verify))
            return 0;

        // evaluate
        for (unsigned j = 0; j < numP; ++j) {
            assert(ec_is_zero(&P12[j].P1) || ec_is_zero(&P12[j].P2));
            if (!gluing_eval_point_special_case(&pts[j], &P12[j], &chain_step.gluing))
                return 0;
        }

        // push kernel points through gluing isogeny
        for (int j = 0; j < current; ++j) {
            gluing_eval_basis(&thetaQ1[j], &thetaQ2[j], &jacQ1[j], &jacQ2[j], &chain_step.gluing);
            if (todo[j] == 0)
                return 0;
            --todo[j];
        }
    ...
    theta.null_point = chain_step.gluing.codomain;
    ...
    for (unsigned i = 1; current >= 0 && todo[current]; ++i) {
        if (current >= THETA_CHAIN_THETA_SLOTS)
            return 0;
        while (todo[current] != 1) {
            uint16_t num_dbls;
            if (!theta_schedule_de2_lookup((unsigned)current, todo[current], &num_dbls) ||
                current + 1 >= THETA_CHAIN_THETA_SLOTS)
                return 0;
            ++current;
        ...
        if (i == n - 2) // penultimate step
            ret = theta_isogeny_compute(
                &chain_step.isogeny, &theta, &thetaQ1[current], &thetaQ2[current], 0, 0, verify);
        else if (i == n - 1) // ultimate step
            ret = theta_isogeny_compute(
                &chain_step.isogeny, &theta, &thetaQ1[current], &thetaQ2[current], 1, 0, false);
        else
            ret = theta_isogeny_compute(
                &chain_step.isogeny, &theta, &thetaQ1[current], &thetaQ2[current], 0, 1, verify);
        if (!ret)
            return 0;

        for (unsigned j = 0; j < numP; ++j)
            theta_isogeny_eval(&pts[j], &chain_step.isogeny, &pts[j]);

        // updating the codomain
        theta = chain_step.isogeny.codomain;

        // pushing the kernel
        for (int j = 0; j < current; ++j) {
            theta_isogeny_eval(&thetaQ1[j], &chain_step.isogeny, &thetaQ1[j]);
            theta_isogeny_eval(&thetaQ2[j], &chain_step.isogeny, &thetaQ2[j]);
            if (todo[j] == 0)
                return 0;
            --todo[j];
        }
    ...
    if (!extra_torsion) {
        if (n >= 3) {
            ...
            theta_isogeny_eval(&thetaQ1[0], &chain_step.isogeny, &thetaQ1[0]);
            theta_isogeny_eval(&thetaQ2[0], &chain_step.isogeny, &thetaQ2[0]);
        }

        // penultimate step
        theta_isogeny_compute_4(&chain_step.isogeny, &theta, &thetaQ1[0], &thetaQ2[0], 0, 0);
        for (unsigned j = 0; j < numP; ++j)
            theta_isogeny_eval(&pts[j], &chain_step.isogeny, &pts[j]);
        theta = chain_step.isogeny.codomain;
        theta_isogeny_eval(&thetaQ1[0], &chain_step.isogeny, &thetaQ1[0]);
        ...
        // ultimate step
        theta_isogeny_compute_2(&chain_step.isogeny, &theta, &thetaQ1[0], &thetaQ2[0], 1, 0);
        for (unsigned j = 0; j < numP; ++j)
            theta_isogeny_eval(&pts[j], &chain_step.isogeny, &pts[j]);
        theta = chain_step.isogeny.codomain;
    }
    ...
    bool is_split =
        splitting_compute(&chain_step.splitting, &theta, extra_torsion ? 8 : -1, randomize);
    ...
    if (!theta_product_structure_to_elliptic_product(E34, &chain_step.splitting.B))
        return 0;

    // evaluate
    for (size_t j = 0; j < numP; ++j) {
        apply_isomorphism(&pts[j], &chain_step.splitting.M, &pts[j]);
        if (!theta_point_to_montgomery_point(&P12[j], &pts[j], &chain_step.splitting.B))
            return 0;
    }
...
#ifdef TARGET_ARM
    theta_chain_workspace_t *workspace =
        (theta_chain_workspace_t *)sqisign_m4_workspace_acquire(sizeof(*workspace));
    if (workspace == NULL)
        return 0;
    int ok = theta_chain_compute_with_workspace(
        n, E12, ker, extra_torsion, E34, P12, numP, verify, randomize, workspace);
    sqisign_m4_workspace_release(workspace);
    return ok;
#else
    theta_chain_workspace_t workspace;
    return theta_chain_compute_with_workspace(
        n, E12, ker, extra_torsion, E34, P12, numP, verify, randomize, &workspace);
#endif
}
