...
#define EC_EVEN_CHAIN_MAX_LENGTH 512
#define EC_EVEN_CHAIN_MAX_SPLITS 10
...
ec_eval_even_strategy(ec_curve_t *curve,
                      ...
                      const int isog_len)
{
    if (curve == NULL || kernel == NULL || isog_len <= 0 ||
        isog_len > EC_EVEN_CHAIN_MAX_LENGTH ||
        (len_points != 0 && points == NULL))
        return (uint32_t)-1;
    ...
    if (space > EC_EVEN_CHAIN_MAX_SPLITS)
        return (uint32_t)-1;
    ec_point_t splits[EC_EVEN_CHAIN_MAX_SPLITS];
    uint16_t todo[EC_EVEN_CHAIN_MAX_SPLITS];
    splits[0] = *kernel;
    todo[0] = isog_len;
