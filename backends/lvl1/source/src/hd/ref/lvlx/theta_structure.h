...
hadamard(theta_point_t *out, const theta_point_t *in)
{
    ...
    /* Publish z+t and z-t only after x,y have been consumed.  The second
     * butterfly then uses output storage, preserving out == in aliasing with
     * two rather than four full fp2 temporaries. */
    fp2_add(&out->x, &in->z, &in->t);
    fp2_sub(&out->y, &in->z, &in->t);
    fp2_sub(&out->z, &t1, &out->x);
    fp2_add(&out->x, &t1, &out->x);
    fp2_sub(&out->t, &t2, &out->y);
    fp2_add(&out->y, &t2, &out->y);
}
