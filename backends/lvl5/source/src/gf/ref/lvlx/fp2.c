...
{
    enum { FP2_BATCHED_INV_MAX = 11 };
    if (x == NULL || len <= 0 || len > FP2_BATCHED_INV_MAX)
        return;

    fp2_t prefix[FP2_BATCHED_INV_MAX];
    fp2_t inverse, output;

    // x = x0,...,xn
    // prefix = x0, x0*x1, ... ,x0 * x1 * ... * xn
    fp2_copy(&prefix[0], &x[0]);
    for (int i = 1; i < len; i++) {
        fp2_mul(&prefix[i], &prefix[i - 1], &x[i]);
    }

    // inverse = 1/ (x0 * x1 * ... * xn)
    fp2_copy(&inverse, &prefix[len - 1]);
    fp2_inv(&inverse);

    /* Walk backwards so one prefix array suffices.  x[i] is consumed before
     * its inverse overwrites it. */
    for (int i = len - 1; i > 0; --i) {
        fp2_mul(&output, &inverse, &prefix[i - 1]);
        fp2_mul(&inverse, &inverse, &x[i]);
        fp2_copy(&x[i], &output);
    }
    fp2_copy(&x[0], &inverse);
}
