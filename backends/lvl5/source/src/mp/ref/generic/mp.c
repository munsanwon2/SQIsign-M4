...
mp_mul(digit_t *c, const digit_t *a, const digit_t *b, size_t nwords)
{
    ...
    assert(nwords >= 2 && nwords <= SQISIGN_MP_MAX_WORDS);
    if (nwords < 2 || nwords > SQISIGN_MP_MAX_WORDS)
        return;
    digit_t carry, UV[2], t[SQISIGN_MP_MAX_WORDS], cc[SQISIGN_MP_MAX_WORDS];
...
{ // Inversion modulo 2^e, using Newton's method and Hensel lifting
    ...
    assert(nwords >= 2 && nwords <= SQISIGN_MP_MAX_WORDS);
    if (nwords < 2 || nwords > SQISIGN_MP_MAX_WORDS || e < 4)
        return;
    digit_t x[SQISIGN_MP_MAX_WORDS], y[SQISIGN_MP_MAX_WORDS];
    digit_t aa[SQISIGN_MP_MAX_WORDS], mp_one[SQISIGN_MP_MAX_WORDS];
    digit_t tmp[SQISIGN_MP_MAX_WORDS];
    mp_copy(aa, a, nwords);
...
mp_invert_matrix(digit_t *r1, digit_t *r2, digit_t *s1, digit_t *s2, int e, unsigned int nwords)
{
    ...
    digit_t det[SQISIGN_MP_MAX_WORDS], tmp[SQISIGN_MP_MAX_WORDS];
    digit_t resa[SQISIGN_MP_MAX_WORDS], resb[SQISIGN_MP_MAX_WORDS];
    digit_t resc[SQISIGN_MP_MAX_WORDS], resd[SQISIGN_MP_MAX_WORDS];
