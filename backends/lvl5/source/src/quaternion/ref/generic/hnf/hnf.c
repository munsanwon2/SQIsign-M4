...
//  book)
void
ibz_mat_4xn_hnf_mod_core(ibz_mat_4x4_t *hnf, int generator_number, ibz_vec_4_t *generators, const ibz_t *mod)
{
    int i = 3;
    ...
    ibz_vec_4_t c;
    /* The algorithm consumes its generator matrix.  All production callers
     * already own mutable scratch and discard it afterwards, so operate in
     * place instead of duplicating up to 16 fixed-width vectors on a nested
     * Cortex-M4 stack frame. */
    ibz_vec_4_t *a = generators;
    ibz_vec_4_t w[4];
    ibz_init(&b);
    ibz_init(&d);
    ibz_init(&u);
