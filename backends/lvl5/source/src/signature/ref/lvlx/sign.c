#include <signature.h>
#include <tools.h>
#include <quaternion_data.h>
#include <id2iso.h>
#include <torsion_constants.h>
#include <encoded_sizes.h>
#include <string.h>

#ifdef TARGET_ARM
#include <sqisign_m4_workspace.h>
#if defined(__GNUC__) || defined(__clang__)
#define SIGNATURE_NOINLINE __attribute__((noinline))
#else
#define SIGNATURE_NOINLINE
#endif

#else
#define SIGNATURE_NOINLINE
#endif

#ifndef SQISIGN_SIGN_MAX_ATTEMPTS
#define SQISIGN_SIGN_MAX_ATTEMPTS 1024u
#endif

typedef enum {
    SIGNATURE_STEP_ML2_EXHAUSTED = -1,
    SIGNATURE_STEP_RETRY = 0,
    SIGNATURE_STEP_OK = 1,
} signature_step_status_t;

typedef struct protocols_sign_post_challenge_storage {
    quat_left_ideal_t lideal_com_resp;
    theta_couple_curve_with_basis_t Eaux2_Echall2;
} protocols_sign_post_challenge_storage_t;

/* The challenge ideal is finalized immediately after the response quaternion
 * is computed.  Only then is post_challenge.lideal_com_resp initialized, and
 * the product-curve result is written later still.  The post-challenge member
 * is at least as large as the challenge ideal, so this union removes the local
 * challenge ideal without increasing the previous persistent workspace. */
typedef union protocols_sign_phase_storage {
    quat_left_ideal_t lideal_chall_two;
    protocols_sign_post_challenge_storage_t post_challenge;
} protocols_sign_phase_storage_t;

_Static_assert(sizeof(protocols_sign_post_challenge_storage_t) >=
                   sizeof(quat_left_ideal_t),
               "phase union must not enlarge the previous signing workspace");

/* Objects that must survive across the signing retry loop.  Keeping them in
 * one target-only arena block removes roughly 25 KiB from the level-V main
 * stack.  The operation guard makes this storage non-reentrant on Cortex-M4;
 * host builds retain an automatic instance for thread-safe tests. */
typedef struct protocols_sign_workspace {
    ibz_t remain;
    ibz_t lattice_content;
    ibz_t random_aux_norm;
    ibz_t degree_resp_inv;
    quat_alg_elem_t resp_quat;
    quat_left_ideal_t lideal_commit;
    theta_couple_curve_with_basis_t Ecom_Eaux;
    protocols_sign_phase_storage_t challenge_or_codomain;
    ec_curve_t E_chall;
} protocols_sign_workspace_t;

#ifdef TARGET_ARM
_Static_assert(sizeof(protocols_sign_workspace_t) <= 25120,
               "signing persistent workspace exceeds the audited bound");
#endif

// compute the commitment with ideal to isogeny clapotis
// and apply it to the basis of E0 (together with the multiplication by some scalar u)
static signature_step_status_t
commit(ec_curve_t *E_com, ec_basis_t *basis_even_com, quat_left_ideal_t *lideal_com)
{
    bool found = false;

    quat_random_ideal_status_t sampling_status =
        quat_sampling_random_ideal_O0_given_norm(
            lideal_com, &COM_DEGREE, 1, &QUAT_represent_integer_params, NULL);
    if (sampling_status == QUAT_RANDOM_IDEAL_FATAL)
        return SIGNATURE_STEP_ML2_EXHAUSTED;
    found = (sampling_status == QUAT_RANDOM_IDEAL_SUCCESS);
    // replacing it with a shorter prime norm equivalent ideal
    found = found && quat_lideal_prime_norm_reduced_equivalent(
                         lideal_com, &QUATALG_PINFTY, QUAT_primality_num_iter, QUAT_equiv_bound_coeff);
    // ideal to isogeny clapotis
    if (found) {
        int isogeny_status =
            dim2id2iso_arbitrary_isogeny_evaluation(basis_even_com, E_com, lideal_com);
        if (isogeny_status == ID2ISO_STATUS_FATAL)
            return SIGNATURE_STEP_ML2_EXHAUSTED;
        found = (isogeny_status > 0);
    }
    return found ? SIGNATURE_STEP_OK : SIGNATURE_STEP_RETRY;
}

static int
compute_challenge_ideal_signature(quat_left_ideal_t *lideal_chall_two, const signature_t *sig, const secret_key_t *sk)
{
    ibz_vec_2_t vec;
    ibz_vec_2_init(&vec);

    // vec is a vector [1, chall_coeff] coefficients encoding the kernel of the challenge
    // isogeny as B[0] + chall_coeff*B[1] where B is the canonical basis of the
    // 2^TORSION_EVEN_POWER torsion of EA
    ibz_set(&vec[0], 1);
    ibz_copy_digit_array(&vec[1], sig->chall_coeff);

    // now we compute the ideal associated to the challenge
    // for that, we need to find vec such that
    // the kernel of the challenge isogeny is generated by vec[0]*B0[0] + vec[1]*B0[1] where B0
    // is the image through the secret key isogeny of the canonical basis E0
    ibz_mat_2x2_eval(&vec, &(sk->mat_BAcan_to_BA0_two), &vec);

    // lideal_chall_two is the pullback of the ideal challenge through the secret key ideal
    int ok = id2iso_kernel_dlogs_to_ideal_even(
        lideal_chall_two, &vec, TORSION_EVEN_POWER);
    if (!ok) {
        ibz_vec_2_finalize(&vec);
        return 0;
    }
    if (ibz_cmp(&lideal_chall_two->norm, &TORSION_PLUS_2POWER) != 0) {
        ibz_vec_2_finalize(&vec);
        return 0;
    }

    ibz_vec_2_finalize(&vec);
    return 1;
}

static int
sample_response(quat_alg_elem_t *x, const quat_lattice_t *lattice, const ibz_t *lattice_content)
{
    ibz_t bound;
    ibz_init(&bound);
    ibz_pow(&bound, &ibz_const_two, SQIsign_response_length);
    ibz_sub(&bound, &bound, &ibz_const_one);
    ibz_mul(&bound, &bound, lattice_content);

    int ok = quat_lattice_sample_from_ball(x, lattice, &QUATALG_PINFTY, &bound);

    ibz_finalize(&bound);
    return ok;
}

static SIGNATURE_NOINLINE signature_step_status_t
compute_response_quat_element(quat_alg_elem_t *resp_quat,
                              ibz_t *lattice_content,
                              const secret_key_t *sk,
                              const quat_left_ideal_t *lideal_chall_two,
                              const quat_left_ideal_t *lideal_commit)
{
    quat_left_ideal_t lideal_chall_secret;
    quat_lattice_t lat_commit;
    ibz_t ideal_norm_gcd;
    signature_step_status_t status = SIGNATURE_STEP_ML2_EXHAUSTED;

    // Init
    quat_left_ideal_init(&lideal_chall_secret);
    quat_lattice_init(&lat_commit);
    ibz_init(&ideal_norm_gcd);

    // lideal_chall_secret = lideal_secret * lideal_chall_two
    if (!quat_lideal_inter(&lideal_chall_secret, lideal_chall_two, &(sk->secret_ideal), &QUATALG_PINFTY)) {
        goto cleanup;
    }

    /* Conjugate(Icom) and lideal_chall_secret are composable integral ideals:
     * O_R(Conjugate(Icom)) = O_0 = O_L(lideal_chall_secret). When their
     * norms are coprime, paper Lemma 13 identifies their intersection with
     * their ordered product, computed by CompactIdealMultiplication. */
    ibz_gcd(&ideal_norm_gcd, &lideal_commit->norm, &lideal_chall_secret.norm);
    if (!ibz_is_one(&ideal_norm_gcd)) {
        status = SIGNATURE_STEP_RETRY;
        goto cleanup;
    }
    quat_lattice_conjugate_without_hnf(&lat_commit, &(lideal_commit->lattice));
    /* Both shortened inputs are private copies inside quat_lattice_mul_mlll,
     * and publication happens only after ML2 succeeds.  Reuse the now-dead
     * challenge-secret lattice for the product instead of retaining a third
     * level-sized lattice in this frame. */
    if (!quat_lattice_mul_mlll(
            &lideal_chall_secret.lattice,
            &lat_commit,
            &lideal_chall_secret.lattice,
            &QUATALG_PINFTY)) {
        goto cleanup;
    }

    // sampling the smallest response
    ibz_mul(lattice_content, &lideal_chall_secret.norm, &lideal_commit->norm);
    if (!sample_response(resp_quat, &lideal_chall_secret.lattice, lattice_content)) {
        status = SIGNATURE_STEP_RETRY;
        goto cleanup;
    }
    status = SIGNATURE_STEP_OK;

    // Clean up
cleanup:
    ibz_finalize(&ideal_norm_gcd);
    quat_left_ideal_finalize(&lideal_chall_secret);
    quat_lattice_finalize(&lat_commit);
    return status;
}

static SIGNATURE_NOINLINE int
compute_backtracking_signature(signature_t *sig, quat_alg_elem_t *resp_quat, ibz_t *lattice_content, ibz_t *remain)
{
    int ok = 0;
    int denom_modified = 0;
    uint_fast8_t backtracking = 0;
    ibz_t tmp;
    ibz_init(&tmp);

    ibz_vec_4_t dummy_coord;
    ibz_vec_4_init(&dummy_coord);

    if (!quat_alg_make_primitive(&dummy_coord, &tmp, resp_quat, &MAXORD_O0)) {
        goto cleanup;
    }

    /* Only the denominator changes.  Keep its rollback value in a dead
     * primitive-coordinate slot instead of copying the five-ibz element. */
    ibz_copy(&dummy_coord[1], &resp_quat->denom);
    ibz_mul(&resp_quat->denom, &resp_quat->denom, &tmp);
    denom_modified = 1;
    // Keep the postcondition explicit in Release builds too.
    if (!quat_lattice_contains(NULL, &MAXORD_O0, resp_quat)) {
        goto cleanup;
    }

    // the backtracking is the common part of the response and the challenge
    // its degree is the scalar tmp computed above such that quat_resp is in tmp * O0.
    int valuation = ibz_two_adic(&tmp);
    if (valuation < 0 || valuation > SQIsign_response_length) {
        goto cleanup;
    }
    backtracking = (uint_fast8_t)valuation;

    /* dummy_coord is dead after primitivization.  Divisibility by 2^k is
     * exactly a two-adic valuation check, and its first entry can hold the
     * exact quotient without a power temporary or generic division. */
    if (ibz_two_adic(lattice_content) < valuation) {
        goto cleanup;
    }
    ibz_div_2exp(&dummy_coord[0], lattice_content, backtracking);

    /* Publish only after every invariant has been checked. */
    ibz_copy(lattice_content, &dummy_coord[0]);
    ibz_set(remain, 0);
    sig->backtracking = backtracking;
    ok = 1;

cleanup:
    if (!ok && denom_modified)
        ibz_copy(&resp_quat->denom, &dummy_coord[1]);
    ibz_finalize(&tmp);
    ibz_vec_4_finalize(&dummy_coord);
    return ok;
}

static SIGNATURE_NOINLINE signature_step_status_t
compute_random_aux_norm_and_helpers(uint_fast8_t *out_pow_dim2_deg_resp,
                                    signature_t *sig,
                                    ibz_t *random_aux_norm,
                                    ibz_t *degree_resp_inv,
                                    ibz_t *remain,
                                    const ibz_t *lattice_content,
                                    quat_alg_elem_t *resp_quat,
                                    quat_left_ideal_t *lideal_com_resp,
                                    quat_left_ideal_t *lideal_commit)
{
    uint_fast8_t pow_dim2_deg_resp;
    uint_fast8_t exp_diadic_val_full_resp;
    signature_step_status_t status = SIGNATURE_STEP_RETRY;

    ibz_t tmp, degree_full_resp, degree_odd_resp, norm_d;

    // Init
    ibz_init(&degree_full_resp);
    ibz_init(&degree_odd_resp);
    ibz_init(&norm_d);
    ibz_init(&tmp);

    if (!quat_alg_norm(&degree_full_resp, &norm_d, resp_quat, &QUATALG_PINFTY))
        goto done;

    // Under NDEBUG the asserts below are compiled out, so a non-unit norm_d or
    // a non-zero remain quietly propagates garbage and starves the outer retry
    // loop. Detect each invariant violation here and signal a retry.
    if (!ibz_is_one(&norm_d)) {
        goto done;
    }

    // dividing by n(lideal_com) * n(lideal_secret_chall)
    ibz_div(&degree_full_resp, remain, &degree_full_resp, lattice_content);
    if (ibz_cmp(remain, &ibz_const_zero) != 0 ||
        ibz_cmp(&degree_full_resp, &ibz_const_zero) <= 0 ||
        sig->backtracking > SQIsign_response_length) {
        goto done;
    }

    // The complete response degree must fit below the configured response
    // bound. This also prevents the exponent arithmetic below from wrapping.
    const int response_bits =
        SQIsign_response_length - sig->backtracking;
    if (ibz_bitsize(&degree_full_resp) > response_bits) {
        goto done;
    }

    // computing the diadic valuation
    int valuation = ibz_two_adic(&degree_full_resp);
    if (valuation < 0 || valuation > SQIsign_response_length - sig->backtracking) {
        goto done;
    }
    exp_diadic_val_full_resp = (uint_fast8_t)valuation;
    sig->two_resp_length = exp_diadic_val_full_resp;

    // removing the power of two part
    ibz_div_2exp(&degree_odd_resp,
                 &degree_full_resp,
                 exp_diadic_val_full_resp);
    // creating the ideal
    quat_alg_conj(resp_quat, resp_quat);

    // setting the norm
    ibz_mul(&tmp, &lideal_commit->norm, &degree_odd_resp);
    /* paper Issue 9: norm = nrd(I_com)·d_rsp known by construction (Sign Line 18) */
    if (!quat_lideal_create_with_norm(
            lideal_com_resp, resp_quat, &tmp, &MAXORD_O0, &QUATALG_PINFTY)) {
        status = SIGNATURE_STEP_ML2_EXHAUSTED;
        goto done;
    }

    // now we compute the ideal_aux
    // computing the norm
    pow_dim2_deg_resp = SQIsign_response_length - exp_diadic_val_full_resp - sig->backtracking;
    ibz_pow(remain, &ibz_const_two, pow_dim2_deg_resp);
    ibz_sub(random_aux_norm, remain, &degree_odd_resp);

    // multiplying by 2^HD_extra_torsion to account for the fact that
    // we use extra torsion above the kernel
    ibz_mul_2exp(remain, remain, HD_extra_torsion);

    if (!ibz_invmod(degree_resp_inv, &degree_odd_resp, remain)) {
        goto done;
    }

    *out_pow_dim2_deg_resp = pow_dim2_deg_resp;
    status = SIGNATURE_STEP_OK;
done:
    ibz_finalize(&degree_full_resp);
    ibz_finalize(&degree_odd_resp);
    ibz_finalize(&norm_d);
    ibz_finalize(&tmp);

    return status;
}

static SIGNATURE_NOINLINE signature_step_status_t
evaluate_random_aux_isogeny_signature(ec_curve_t *E_aux,
                                      ec_basis_t *B_aux,
                                      const ibz_t *norm,
                                      quat_left_ideal_t *lideal_aux,
                                      quat_left_ideal_t *lideal_aux_resp_com)
{
    signature_step_status_t status = SIGNATURE_STEP_RETRY;

    // sampling the ideal at random
    quat_random_ideal_status_t sampling_status =
        quat_sampling_random_ideal_O0_given_norm(
            lideal_aux, norm, 0, &QUAT_represent_integer_params, &QUAT_prime_cofactor);
    if (sampling_status == QUAT_RANDOM_IDEAL_FATAL) {
        status = SIGNATURE_STEP_ML2_EXHAUSTED;
        goto cleanup;
    }
    int found = (sampling_status == QUAT_RANDOM_IDEAL_SUCCESS);

    if (!found)
        goto cleanup;

    // pushing forward
    /* Publication in quat_lideal_inter is transactional, so the result may
     * alias its first input.  The caller deliberately supplies the now-dead
     * commitment ideal as lideal_aux and reuses lideal_com_resp in place.
     * This removes two nested lvl-V ideal objects (about 15 KiB) from the
     * Cortex-M4 stack without increasing the CCM arena. */
    if (!quat_lideal_inter(
            lideal_aux_resp_com, lideal_aux_resp_com, lideal_aux, &QUATALG_PINFTY)) {
        status = SIGNATURE_STEP_ML2_EXHAUSTED;
        goto cleanup;
    }

    // now we evaluate this isogeny on the basis of E0
    int isogeny_status =
        dim2id2iso_arbitrary_isogeny_evaluation(B_aux, E_aux, lideal_aux_resp_com);
    if (isogeny_status == ID2ISO_STATUS_FATAL) {
        status = SIGNATURE_STEP_ML2_EXHAUSTED;
        goto cleanup;
    }
    found = (isogeny_status > 0);

    status = found ? SIGNATURE_STEP_OK : SIGNATURE_STEP_RETRY;

cleanup:
    return status;
}

static SIGNATURE_NOINLINE int
compute_dim2_isogeny_challenge(theta_couple_curve_with_basis_t *codomain,
                               theta_couple_curve_with_basis_t *domain,
                               const ibz_t *degree_resp_inv,
                               int pow_dim2_deg_resp,
                               int exp_diadic_val_full_resp,
                               int reduced_order)
{
    // now, we compute the isogeny Phi : Ecom x Eaux -> Echl' x Eaux'
    // where Echl' is 2^exp_diadic_val_full_resp isogenous to Echal
    // ker Phi = <(Bcom_can.P,Baux.P),(Bcom_can.Q,Baux.Q)>

    // preparing the domain
    theta_couple_curve_t EcomXEaux;
    copy_curve(&EcomXEaux.E1, &domain->E1);
    copy_curve(&EcomXEaux.E2, &domain->E2);

    // preparing the kernel
    theta_kernel_couple_points_t dim_two_ker;
    copy_bases_to_kernel(&dim_two_ker, &domain->B1, &domain->B2);

    // dividing by the degree of the response
    digit_t scalar[NWORDS_ORDER];
    if (!ibz_to_mp_digits_checked(scalar, NWORDS_ORDER, degree_resp_inv)) {
        return 0;
    }
    ec_mul(&dim_two_ker.T1.P2, scalar, reduced_order, &dim_two_ker.T1.P2, &EcomXEaux.E2);
    ec_mul(&dim_two_ker.T2.P2, scalar, reduced_order, &dim_two_ker.T2.P2, &EcomXEaux.E2);
    ec_mul(&dim_two_ker.T1m2.P2, scalar, reduced_order, &dim_two_ker.T1m2.P2, &EcomXEaux.E2);

    // and multiplying by 2^exp_diadic...
    double_couple_point_iter(&dim_two_ker.T1, exp_diadic_val_full_resp, &dim_two_ker.T1, &EcomXEaux);
    double_couple_point_iter(&dim_two_ker.T2, exp_diadic_val_full_resp, &dim_two_ker.T2, &EcomXEaux);
    double_couple_point_iter(&dim_two_ker.T1m2, exp_diadic_val_full_resp, &dim_two_ker.T1m2, &EcomXEaux);

    theta_couple_point_t pushed_points[3];
    theta_couple_point_t *const Tev1 = pushed_points + 0, *const Tev2 = pushed_points + 1,
                                *const Tev1m2 = pushed_points + 2;

    // Set points on the commitment curve
    copy_point(&Tev1->P1, &domain->B1.P);
    copy_point(&Tev2->P1, &domain->B1.Q);
    copy_point(&Tev1m2->P1, &domain->B1.PmQ);

    // Zero points on the aux curve
    ec_point_init(&Tev1->P2);
    ec_point_init(&Tev2->P2);
    ec_point_init(&Tev1m2->P2);

    theta_couple_curve_t codomain_product;

    // computation of the dim2 isogeny
    if (!theta_chain_compute_and_eval_randomized(pow_dim2_deg_resp,
                                                 &EcomXEaux,
                                                 &dim_two_ker,
                                                 true,
                                                 &codomain_product,
                                                 pushed_points,
                                                 sizeof(pushed_points) / sizeof(*pushed_points)))
        return 0;

    assert(test_couple_point_order_twof(Tev1, &codomain_product, reduced_order));

    // Set the auxiliary curve
    copy_curve(&codomain->E1, &codomain_product.E2);

    // Set the codomain curve from the dim 2 isogeny
    // it should always be the first curve
    copy_curve(&codomain->E2, &codomain_product.E1);

    // Set the evaluated basis points
    copy_point(&codomain->B1.P, &Tev1->P2);
    copy_point(&codomain->B1.Q, &Tev2->P2);
    copy_point(&codomain->B1.PmQ, &Tev1m2->P2);

    copy_point(&codomain->B2.P, &Tev1->P1);
    copy_point(&codomain->B2.Q, &Tev2->P1);
    copy_point(&codomain->B2.PmQ, &Tev1m2->P1);
    return 1;
}

static SIGNATURE_NOINLINE int
compute_small_chain_isogeny_signature(ec_curve_t *E_chall_2,
                                      ec_basis_t *B_chall_2,
                                      const quat_alg_elem_t *resp_quat,
                                      int pow_dim2_deg_resp,
                                      int length)
{
    int ret = 1;

    ibz_t two_pow;
    ibz_init(&two_pow);

    ibz_vec_2_t vec_resp_two;
    ibz_vec_2_init(&vec_resp_two);

    quat_left_ideal_t lideal_resp_two;
    quat_left_ideal_init(&lideal_resp_two);

    // computing the ideal
    ibz_pow(&two_pow, &ibz_const_two, length);

    // we compute the generator of the challenge ideal
    /* paper Issue 9: norm = 2^length known by construction */
    if (!quat_lideal_create_with_norm(
            &lideal_resp_two, resp_quat, &two_pow, &MAXORD_O0, &QUATALG_PINFTY)) {
        ret = 0;
        goto cleanup;
    }

    // computing the coefficients of the kernel in terms of the basis of O0
    if (!id2iso_ideal_to_kernel_dlogs_even(&vec_resp_two, &lideal_resp_two)) {
        ret = 0;
        goto cleanup;
    }

    ec_point_t points[3];
    copy_point(&points[0], &B_chall_2->P);
    copy_point(&points[1], &B_chall_2->Q);
    copy_point(&points[2], &B_chall_2->PmQ);

    // getting down to the right order and applying the matrix
    ec_dbl_iter_basis(B_chall_2, pow_dim2_deg_resp + HD_extra_torsion, B_chall_2, E_chall_2);
    assert(test_basis_order_twof(B_chall_2, E_chall_2, length));

    ec_point_t ker;
    // applying the vector to find the kernel
    if (!ec_biscalar_mul_ibz_vec(&ker, &vec_resp_two, length, B_chall_2, E_chall_2)) {
        ret = 0;
        goto cleanup;
    }
    assert(test_point_order_twof(&ker, E_chall_2, length));

    // computing the isogeny and pushing the points
    if (ec_eval_small_chain(E_chall_2, &ker, length, points, 3, true)) {
        ret = 0;
    }

    // copying the result
    copy_point(&B_chall_2->P, &points[0]);
    copy_point(&B_chall_2->Q, &points[1]);
    copy_point(&B_chall_2->PmQ, &points[2]);

cleanup:
    ibz_finalize(&two_pow);
    ibz_vec_2_finalize(&vec_resp_two);
    quat_left_ideal_finalize(&lideal_resp_two);

    return ret;
}

static SIGNATURE_NOINLINE int
compute_challenge_codomain_signature(const signature_t *sig,
                                     secret_key_t *sk,
                                     ec_curve_t *E_chall,
                                     const ec_curve_t *E_chall_2,
                                     ec_basis_t *B_chall_2)
{
    ec_isog_even_t phi_chall;
    ec_basis_t bas_sk;
    copy_basis(&bas_sk, &sk->canonical_basis);

    phi_chall.curve = sk->curve;
    phi_chall.length = TORSION_EVEN_POWER - sig->backtracking;
    assert(test_basis_order_twof(&bas_sk, &sk->curve, TORSION_EVEN_POWER));

    // Compute the kernel
    if (!ec_ladder3pt(
            &phi_chall.kernel, sig->chall_coeff, &bas_sk.P, &bas_sk.Q, &bas_sk.PmQ, &sk->curve))
        return 0;
    assert(test_point_order_twof(&phi_chall.kernel, &sk->curve, TORSION_EVEN_POWER));

    // Double kernel to get correct order
    ec_dbl_iter(&phi_chall.kernel, sig->backtracking, &phi_chall.kernel, &sk->curve);

    assert(test_point_order_twof(&phi_chall.kernel, E_chall, phi_chall.length));

    // Compute the codomain from challenge isogeny
    if (ec_eval_even(E_chall, &phi_chall, NULL, 0))
        return 0;

#ifndef NDEBUG
    fp2_t j_chall, j_codomain;
    ec_j_inv(&j_codomain, E_chall_2);
    ec_j_inv(&j_chall, E_chall);
    // apparently its always the second one curve
    assert(fp2_is_equal(&j_chall, &j_codomain));
#endif

    // applying the isomorphism from E_chall_2 to E_chall
    ec_isom_t isom;
    if (ec_isomorphism(&isom, E_chall_2, E_chall))
        return 0; // error due to a corner case with 1/p probability
    ec_iso_eval(&B_chall_2->P, &isom);
    ec_iso_eval(&B_chall_2->Q, &isom);
    ec_iso_eval(&B_chall_2->PmQ, &isom);

    return 1;
}

static void
set_aux_curve_signature(signature_t *sig, ec_curve_t *E_aux)
{
    ec_normalize_curve(E_aux);
    fp2_copy(&sig->E_aux_A, &E_aux->A);
}

static SIGNATURE_NOINLINE int
compute_and_set_basis_change_matrix(signature_t *sig,
                                    const ec_basis_t *B_aux_2,
                                    ec_basis_t *B_chall_2,
                                    ec_curve_t *E_aux_2,
                                    ec_curve_t *E_chall,
                                    int f)
{
    int ret = 0;

    // Matrices for change of bases matrices
    ibz_mat_2x2_t mat_Baux2_to_Baux2_can, mat_Bchall_can_to_Bchall;
    ibz_mat_2x2_init(&mat_Baux2_to_Baux2_can);
    ibz_mat_2x2_init(&mat_Bchall_can_to_Bchall);

    // Compute canonical bases
    ec_basis_t B_can_chall, B_aux_2_can;
    if (!ec_curve_to_basis_2f_to_hint(
            &B_can_chall, E_chall, TORSION_EVEN_POWER, &sig->hint_chall) ||
        !ec_curve_to_basis_2f_to_hint(
            &B_aux_2_can, E_aux_2, TORSION_EVEN_POWER, &sig->hint_aux)) {
        goto cleanup;
    }

#ifndef NDEBUG
    {
        // Ensure all points have the desired order
        assert(test_basis_order_twof(&B_aux_2_can, E_aux_2, TORSION_EVEN_POWER));
        assert(test_basis_order_twof(B_aux_2, E_aux_2, f));
        fp2_t w0;
        weil(&w0, f, &B_aux_2->P, &B_aux_2->Q, &B_aux_2->PmQ, E_aux_2);
    }
#endif

    // compute the matrix to go from B_aux_2 to B_aux_2_can
    change_of_basis_matrix_tate_invert(&mat_Baux2_to_Baux2_can, &B_aux_2_can, B_aux_2, E_aux_2, f);

    // apply the change of basis to B_chall_2
    if (!matrix_application_even_basis(B_chall_2, E_chall, &mat_Baux2_to_Baux2_can, f))
        goto cleanup;

#ifndef NDEBUG
    {
        // Ensure all points have the desired order
        assert(test_basis_order_twof(&B_can_chall, E_chall, TORSION_EVEN_POWER));
    }
#endif

    // compute the matrix to go from B_chall_can to B_chall_2
    change_of_basis_matrix_tate(&mat_Bchall_can_to_Bchall, B_chall_2, &B_can_chall, E_chall, f);

    // All values must be non-negative and fit the serialized scalar capacity.
    for (int row = 0; row < 2; ++row) {
        for (int col = 0; col < 2; ++col) {
            const ibz_t *entry = &mat_Bchall_can_to_Bchall[row][col];
            if (ibz_is_negative(entry) ||
                ibz_bitsize(entry) > SQIsign_response_length + HD_extra_torsion) {
                goto cleanup;
            }
        }
    }

    // Set the basis change matrix to signature
    for (int row = 0; row < 2; ++row) {
        for (int col = 0; col < 2; ++col) {
            if (!ibz_to_mp_digits_checked(sig->mat_Bchall_can_to_B_chall[row][col],
                                       NWORDS_ORDER,
                                       &mat_Bchall_can_to_Bchall[row][col])) {
                memset(sig->mat_Bchall_can_to_B_chall, 0, sizeof(sig->mat_Bchall_can_to_B_chall));
                goto cleanup;
            }
        }
    }
    ret = 1;

    // Finalise the matrices
cleanup:
    ibz_mat_2x2_finalize(&mat_Bchall_can_to_Bchall);
    ibz_mat_2x2_finalize(&mat_Baux2_to_Baux2_can);
    return ret;
}

int
protocols_sign(signature_t *sig, const public_key_t *pk, secret_key_t *sk, const unsigned char *m, size_t l)
{
    int ret = 0;
    int reduced_order = 0; // work around false positive gcc warning

#ifdef TARGET_ARM
    protocols_sign_workspace_t *workspace =
        (protocols_sign_workspace_t *)sqisign_m4_workspace_acquire(sizeof(*workspace));
    if (workspace == NULL)
        return 0;
#else
    protocols_sign_workspace_t automatic_workspace;
    protocols_sign_workspace_t *workspace = &automatic_workspace;
#endif

    ibz_t *const remain = &workspace->remain;
    ibz_t *const lattice_content = &workspace->lattice_content;
    ibz_t *const random_aux_norm = &workspace->random_aux_norm;
    ibz_t *const degree_resp_inv = &workspace->degree_resp_inv;
    quat_alg_elem_t *const resp_quat = &workspace->resp_quat;
    quat_left_ideal_t *const lideal_commit = &workspace->lideal_commit;
    quat_left_ideal_t *const lideal_com_resp =
        &workspace->challenge_or_codomain.post_challenge.lideal_com_resp;
    theta_couple_curve_with_basis_t *const Ecom_Eaux = &workspace->Ecom_Eaux;
    theta_couple_curve_with_basis_t *const Eaux2_Echall2 =
        &workspace->challenge_or_codomain.post_challenge.Eaux2_Echall2;
    ec_curve_t *const E_chall = &workspace->E_chall;

    uint_fast8_t pow_dim2_deg_resp;
    assert(SQIsign_response_length <= (intmax_t)UINT_FAST8_MAX); // otherwise we might need more bits there

    ibz_init(remain);
    ibz_init(lattice_content);
    ibz_init(random_aux_norm);
    ibz_init(degree_resp_inv);

    quat_alg_elem_init(resp_quat);

    quat_left_ideal_init(lideal_commit);

    // This will hold the challenge curve.
    copy_curve(E_chall, &sk->curve);

    ec_curve_init(&Ecom_Eaux->E1);
    ec_curve_init(&Ecom_Eaux->E2);

    for (unsigned signing_attempt = 0;
         !ret && signing_attempt < SQISIGN_SIGN_MAX_ATTEMPTS;
         ++signing_attempt) {

        // computing the commitment
        signature_step_status_t commit_status =
            commit(&Ecom_Eaux->E1, &Ecom_Eaux->B1, lideal_commit);
        if (commit_status == SIGNATURE_STEP_ML2_EXHAUSTED) {
            ret = 0;
            goto cleanup;
        }
        ret = (commit_status == SIGNATURE_STEP_OK);
        (void)0;

        // start again if the commitment generation has failed
        if (!ret) {
            continue;
        }

        // Hash the message to a kernel generator
        // i.e. a scalar such that ker = P + [s]Q
        hash_to_challenge(&sig->chall_coeff, pk, &Ecom_Eaux->E1, m, l);
        // Compute the challenge ideal and response quaternion element
        {
            quat_left_ideal_t *const lideal_chall_two =
                &workspace->challenge_or_codomain.lideal_chall_two;
            quat_left_ideal_init(lideal_chall_two);

            // computing the challenge ideal
            if (!compute_challenge_ideal_signature(lideal_chall_two, sig, sk)) {
                quat_left_ideal_finalize(lideal_chall_two);
                ret = 0;
                goto cleanup;
            }
            (void)0;
            signature_step_status_t response_status = compute_response_quat_element(
                resp_quat, lattice_content, sk, lideal_chall_two, lideal_commit);

            // Clean up
            quat_left_ideal_finalize(lideal_chall_two);

            if (response_status == SIGNATURE_STEP_ML2_EXHAUSTED) {
                ret = 0;
                goto cleanup;
            }
            if (response_status != SIGNATURE_STEP_OK) {
                ret = 0;
                continue;
            }
        }

        // computing the amount of backtracking we're making
        // and removing it
        if (!compute_backtracking_signature(sig, resp_quat, lattice_content, remain)) {
            // make_primitive's lattice-containment invariant was violated (silently
            // under NDEBUG). Retry from commit instead of spinning forever.
            ret = 0;
            continue;
        }
        (void)0;

        // creating lideal_com * lideal_resp
        // we first compute the norm of lideal_resp
        // norm of the resp_quat
        /* This begins the post-challenge union lifetime.  Every branch below
         * finalizes the ideal before retrying, returning, or reusing the union
         * for the next attempt's challenge ideal. */
        quat_left_ideal_init(lideal_com_resp);
        signature_step_status_t helper_status =
            compute_random_aux_norm_and_helpers(&pow_dim2_deg_resp,
                                                sig,
                                                random_aux_norm,
                                                degree_resp_inv,
                                                remain,
                                                lattice_content,
                                                resp_quat,
                                                lideal_com_resp,
                                                lideal_commit);
        if (helper_status == SIGNATURE_STEP_ML2_EXHAUSTED) {
            quat_left_ideal_finalize(lideal_com_resp);
            ret = 0;
            goto cleanup;
        }
        if (helper_status != SIGNATURE_STEP_OK) {
            // Norm/divisibility invariants violated under NDEBUG; retry from commit.
            quat_left_ideal_finalize(lideal_com_resp);
            ret = 0;
            continue;
        }
        (void)0;

        // notational conventions:
        // B0 = canonical basis of E0
        // B_com = image through commitment isogeny (odd degree) of canonical basis of E0
        // B_aux = image through aux_resp_com isogeny (odd degree) of canonical basis of E0

        if (pow_dim2_deg_resp > 0) {
            // Evaluate the random aux ideal on the curve E0 and its basis to find E_aux and B_aux
            /* lideal_commit is dead after response construction.  The helper
             * also consumes lideal_com_resp in place, and both arena objects
             * are rebuilt before the next signing retry. */
            signature_step_status_t aux_status = evaluate_random_aux_isogeny_signature(
                &Ecom_Eaux->E2,
                &Ecom_Eaux->B2,
                random_aux_norm,
                lideal_commit,
                lideal_com_resp);
            (void)0;

            /* The arbitrary-isogeny evaluation has consumed the ideal. */
            quat_left_ideal_finalize(lideal_com_resp);

            if (aux_status == SIGNATURE_STEP_ML2_EXHAUSTED) {
                ret = 0;
                goto cleanup;
            }
            ret = (aux_status == SIGNATURE_STEP_OK);

            // auxiliary isogeny computation failed we must start again
            if (!ret) {
                continue;
            }

#ifndef NDEBUG
            // testing that the order of the points in the bases is as expected
            assert(test_basis_order_twof(&Ecom_Eaux->B1, &Ecom_Eaux->E1, TORSION_EVEN_POWER));
            assert(test_basis_order_twof(&Ecom_Eaux->B2, &Ecom_Eaux->E2, TORSION_EVEN_POWER));
#endif

            // applying the matrix to compute Baux
            // first, we reduce to the relevant order
            reduced_order = pow_dim2_deg_resp + HD_extra_torsion + sig->two_resp_length;
            ec_dbl_iter_basis(
                &Ecom_Eaux->B1, TORSION_EVEN_POWER - reduced_order, &Ecom_Eaux->B1, &Ecom_Eaux->E1);
            ec_dbl_iter_basis(
                &Ecom_Eaux->B2, TORSION_EVEN_POWER - reduced_order, &Ecom_Eaux->B2, &Ecom_Eaux->E2);

            // Given all the above data, compute a dim two isogeny with domain
            // E_com x E_aux
            // and codomain
            // E_aux_2 x E_chall_2 (note: E_chall_2 is isomorphic to E_chall)
            // and evaluated points stored as bases in
            // B_aux_2 on E_aux_2
            // B_chall_2 on E_chall_2
            ret = compute_dim2_isogeny_challenge(Eaux2_Echall2,
                                                  Ecom_Eaux,
                                                  degree_resp_inv,
                                                  pow_dim2_deg_resp,
                                                  sig->two_resp_length,
                                                  reduced_order);
            (void)0;
            if (!ret) {
                continue;
            }
        } else {
            /* No auxiliary ideal evaluation is needed on this branch. */
            quat_left_ideal_finalize(lideal_com_resp);

            // No 2d isogeny needed, so simulate a "Kani matrix" identity here
            copy_curve(&Eaux2_Echall2->E1, &Ecom_Eaux->E1);
            copy_curve(&Eaux2_Echall2->E2, &Ecom_Eaux->E1);

            reduced_order = sig->two_resp_length;
            ec_dbl_iter_basis(&Eaux2_Echall2->B1,
                              TORSION_EVEN_POWER - reduced_order,
                              &Ecom_Eaux->B1,
                              &Ecom_Eaux->E1);
            copy_basis(&Eaux2_Echall2->B2, &Eaux2_Echall2->B1);
        }

        // computation of the remaining small chain of two isogenies when needed
        if (sig->two_resp_length > 0) {
            if (!compute_small_chain_isogeny_signature(&Eaux2_Echall2->E2,
                                                        &Eaux2_Echall2->B2,
                                                        resp_quat,
                                                        pow_dim2_deg_resp,
                                                        sig->two_resp_length)) {
                ret = 0;
                goto cleanup;
            }
        }

        // computation of the challenge codomain
        if (!compute_challenge_codomain_signature(
                sig, sk, E_chall, &Eaux2_Echall2->E2, &Eaux2_Echall2->B2)) {
            // The isomorphism computation has a documented low-probability
            // exceptional case. Start with a fresh commitment and restore the
            // challenge curve on the next iteration.
            ret = 0;
            copy_curve(E_chall, &sk->curve);
            continue;
        }
    }

    if (!ret)
        goto cleanup;

    // Set to the signature the Montgomery A-coefficient of E_aux_2
    set_aux_curve_signature(sig, &Eaux2_Echall2->E1);

    // Set the basis change matrix from canonical bases to the supplied bases
    if (!compute_and_set_basis_change_matrix(sig,
                                             &Eaux2_Echall2->B1,
                                             &Eaux2_Echall2->B2,
                                             &Eaux2_Echall2->E1,
                                             E_chall,
                                             reduced_order)) {
        ret = 0;
    }

cleanup:
    quat_alg_elem_finalize(resp_quat);
    quat_left_ideal_finalize(lideal_commit);

    ibz_finalize(lattice_content);
    ibz_finalize(remain);
    ibz_finalize(degree_resp_inv);
    ibz_finalize(random_aux_norm);

#ifdef TARGET_ARM
    sqisign_m4_workspace_release(workspace);
#endif

    return ret;
}
